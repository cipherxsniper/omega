...
print("[Ω] booting v7.18 attention + predictive layer...", flush=True)
...
while True:
    ...
    # 🧠 ATTENTION SCORING
    attn_score = attention.score_event(event, graph.snapshot())
    ...
    print(f"\n[Ω v7.18 | TICK {tick}]", flush=True)
    print("ATTENTION:", attn_score, flush=True)
    print("RISK:", risk_level, flush=True)
    print("QUEUE:", scheduler.size(), flush=True)
